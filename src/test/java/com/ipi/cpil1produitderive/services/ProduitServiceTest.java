...
public class ProduitServiceTest {
    ...
    @Test
    public void testGetVentesProduit() throws Exception {
        ...
        //given
        Mockito.when(produitDAO.findById(1L)).thenReturn(Optional.of(produit1));
        //when
        VentesProduit ventesProduit =  produitService.getVentesProduit(1L);
        //then
        Assertions.assertThat(ventesProduit.getProduit().getNom()).isEqualTo("P1");
        Assertions.assertThat(ventesProduit.getQuantiteVendu()).isEqualTo(2);
        Assertions.assertThat(ventesProduit.getPrixTotal()).isEqualTo(16.0);
    }

    @Test
    public void testGetVentesProduitWithWrongFamilleId(){
        //given
        Mockito.when(produitDAO.findById(1L)).thenReturn(Optional.empty());
        //when
        Throwable t = Assertions.catchThrowable(() -> {
            VentesProduit ventesProduit = produitService.getVentesProduit(1L);
        });
        //then
        Assertions.assertThat(t).hasMessage("Produit non trouvée.");
    }

    @Test
    public void testGetVentesProduitWithoutCommande() throws Exception {
        ...
        //then
        Assertions.assertThat(ventesProduit.getProduit().getNom()).isEqualTo("P1");
        Assertions.assertThat(ventesProduit.quantiteVendu).isEqualTo(0);
        Assertions.assertThat(ventesProduit.prixTotal).isEqualTo(0.0);
    }

    @Test
    public void testGetVentesProduitWithoutValideCommande() throws Exception {
